...
create procedure [dbo].[spMergePricingCommunityRate] as begin
	...
	declare @ProcessName varchar(100) = 'spMergePricingCommunityRate';
	...
	(
		...
		FROM [CPS_DW_STAGE].[dbo].[PricingCommunityRates] pcr
	...
	merge into [CPS_DW].[dbo].[PricingCommunityRate] as tgt -- target table
	using (
		select
			[RateYear]
			,[BenefitPlanId]
	...
	on tgt.[RateYear] = src.[RateYear]
		...
		and tgt.[CostAreaValue] = src.[CostAreaValue]
