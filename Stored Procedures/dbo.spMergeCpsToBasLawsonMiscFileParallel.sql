...
CREATE procedure [dbo].[spMergeCpsToBasLawsonMiscFileParallel] as begin
	...
	declare @ProcessName varchar(100) = 'spMergeCpsToBasLawsonMiscFileParallel'
	declare @ExecutingUser varchar(100) = SUSER_NAME()
	
	merge into [CPS_DW_STAGE].[dbo].[CpsToBasLawsonMiscFileParallel] as tgt -- target table
	using (
		SELECT
			[LINE ID]
			,[EMPLOYER ID]
		...
		FROM [CPS_DW].[bas].[vBasMiscDataFileParallel]
